...
import { testfetch,submitfetch, deleteItem,apiTestFetch, scrapeSteam, syncSkinport, updateCurrencyRatio,item_row, getAllItems } from "./services/api";
    ...
    <div className="mt-7 flex overflow-x-auto rounded-lg shadow-2xl">
      <table className="w-full border-collapse text-left">
        ...
        <tbody className="bg-[#001219]">
          {items.map((item)=>(
            <tr key={item.name} className="transition-colors border-b border-black hover:bg-[#531315] hover:border-5">
              ...
              <td className="items-center p-3">
                {new Date(item.steam_updated).toLocaleTimeString()}
              </td>
              <td className="items-center p-3">
                {new Date(item.skinport_updated).toLocaleTimeString()}
              </td>
            </tr>
          ))}
        </tbody>
